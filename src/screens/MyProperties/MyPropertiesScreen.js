...
import { useGetMeQuery, useGetMyPropertiesQuery } from '../../services/authApi';
...
export default function MyPropertiesScreen() {
  ...
  const { data: userData, isLoading: userPropertiesLoading } =
  ...
  const { data: agentData, isLoading: agentPropertiesLoading } =
    useGetAgentPropertiesQuery(isAgent ? user?.agentProfileId : skipToken);
  ...
  const isLoading =
    userLoading || userPropertiesLoading || agentPropertiesLoading;
  ...
  const properties = useMemo(() => {
    ...
    if (filter !== 'all') {
      list = list.filter(item => {
        ...
        return (
          item.approvalStatus === 'approved' && item.listingStatus === filter
        );
      });
    }
    ...
    return list;
  }, [data, search, filter, sort, isAgent]);
  ...
  const filters = isAgent
    ...
    : ['all', 'available', 'rented', 'sold', 'pending', 'rejected'];
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <OwnerPropertyCard
            ...
            onEdit={() => navigation.navigate('EditProperty', { id: item._id })}
          />
        )}
